...
abstract class BaseListView extends Widget
{
    ...
    public function init()
    {
        ...
        if ($this->emptyText === null) {
            $this->emptyText = Yii::t('yii', 'Ничего не найдено по запросу.');
        }
    ...
    public function renderSummary()
    {
        ...
        if (($pagination = $this->dataProvider->getPagination()) !== false) {
            ...
            if (($summaryContent = $this->summary) === null) {
                //return Html::tag($tag, Yii::t('yii', 'Найдено <b>{begin, number}-{end, number}</b> результатов из <b>{totalCount, number}</b> {totalCount, plural, one{item} other{items}}.', [
				return Html::tag($tag, Yii::t('yii', 'Найдено <b>{totalCount, number}</b> {totalCount, plural, one{строка} other{строк}} результатов.', [
                        'begin' => $begin,
                        'end' => $end,
                        'count' => $count,
